...
def combine_psk_and_gltf(psk_name, gltf_name):
    ...
    #get objects
    psk_armature, psk_mesh, psk_materials, psk_uv_names = get_armature(psk_name)
    gltf_armature, gltf_mesh, gltf_materials, gltf_uv_names = get_armature(gltf_name)
    
    psk_vert_num = len(psk_mesh.data.vertices)
    gltf_vert_num = len(gltf_mesh.data.vertices)
